...
extension PoseVariant {
    ...
    static let earSideProfile = PoseVariant(
        ...
        icon: "arrow.left.to.line.compact",
        category: .angle,
        promptSuffix: "STRICT SIDE PROFILE: rotate the head a full 90 degrees so only one side of the face is visible, nose pointing to the side of the frame, the full ear visible and centered, the earring sharp and in focus, clean background, close-up beauty-portrait framing with soft jewelry-catalog lighting. PROFILE SCALE RULE: this is a tighter crop of the same real head, not a bigger jewel. Zooming in enlarges the ear and the earring together and never changes the ratio between them, so the piece keeps exactly the size it would have in a wide shot - measured against this wearer's ear (about 6 cm tall, earlobe about 1.5 cm) with its metal wire about 1 mm thick. A stud reads as a few millimeters of metal on the lobe. A drop earring keeps its own full length, whatever that length is, and may hang below the jawline onto the neck. IF A PRODUCT REFERENCE IMAGE IS PROVIDED: the earring is exactly that product - same design, same length, same proportions, same materials, same stones. Only one ear is visible in this pose, so render the piece of the pair that belongs to that side. Never swap it for a smaller, simpler, generic or hoop-like earring, and never shorten, shrink or crop it, in order to fit this framing. If it does not fit, widen the crop to include the jaw, neck and shoulder. Losing the product's identity is the worst possible outcome of this render. ONLY ONE EARRING IS VISIBLE: in a strict profile the far ear is hidden behind the head, so its earring is not visible either. Draw exactly one earring, worn on the ear facing the camera. Never draw a second earring floating in the air, detached from the head, or overlapping the jaw or neck.",
        itemTypes: [.earrings],
        isMotion: false
    )
