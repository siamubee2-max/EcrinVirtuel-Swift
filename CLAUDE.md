...
- Edge functions (all 5 versioned in `supabase/functions/` — keep them in the repo): `tryon-generate` (fal.ai cascade: standard → `fal-ai/nano-banana-2/edit` 9:16, premium → `fal-ai/gemini-3-pro-image-preview/edit` (NB Pro); fallbacks flux-pro/kontext → Gemini 3.1 Flash Image direct → OpenAI gpt-image-1; founder bypass `siamubee2@gmail.com` / `monia.valenza@gmail.com`), `credit-generations` (starter=15/premium=40/elite=100, idempotent via `credit_transactions`), `delete-user-account`, `styliste-chat` (OpenAI → Gemini), `revenuecat-webhook` (requires `RC_WEBHOOK_SECRET`, verify_jwt off).
